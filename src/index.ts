...
function updateCartTotalPlus(price: string, updateType: string): void {
  ...
  price = price.replace(",", ".");
  let updatedPricePlus: number = (numberTotalValue += Number(
    price.substring(1)
  ));
  let updatedPriceMinus: number = (numberTotalValue -= Number(
    price.substring(1)
  ));
  if (updateType == "+") {
